-- Promo CD + tee (−5 €) + meilleure auto-promo (pas de cumul).

alter table public.promo_codes
  add column if not exists min_cd_qty integer not null default 0
    check (min_cd_qty >= 0);

comment on column public.promo_codes.min_cd_qty is
  'Quantité minimale de CD dans le panier (0 = pas de contrainte).';
...
on conflict (code) do update set
  ...
  min_cd_qty = excluded.min_cd_qty,
...
begin
  ...
  if promo_requested then
    ...
    -- Une seule auto-promo : la meilleure réduction éligible (pas de cumul).
    select * into promo_row
    from public.promo_codes
    where id = (
      select p.id
