-- Tailles : CD / VINYL (formats) + ENF (tee enfant). U conservé pour l’historique commandes.
...
alter table public.product_variants
  add constraint product_variants_size_check check (
    size = any (array['XS'::text, 'S'::text, 'M'::text, 'L'::text, 'XL'::text, 'ENF'::text, 'CD'::text, 'VINYL'::text, 'U'::text])
  );

-- CD : remplacer U → CD sur le stock catalogue (pas les order_items historiques).
update public.product_variants
set size = 'CD'
where size = 'U';

-- Taille enfant à 0 pour tous les tees.
insert into public.product_variants (product_id, size, stock, reserved)
select p.id, 'ENF', 0, 0
from public.products p
where coalesce(p.type_key, '') = 'tshirt'
   or p.type = 'T-shirt'
...
begin
  ...
  loop
    ...
    if v_size not in ('XS', 'S', 'M', 'L', 'XL', 'ENF', 'CD', 'VINYL', 'U') then
