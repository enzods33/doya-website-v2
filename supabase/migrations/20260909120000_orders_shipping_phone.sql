-- Store shipping phone from Stripe Checkout (phone_number_collection).

alter table public.orders
  add column if not exists shipping_phone text;

drop function if exists public.mark_order_paid_from_stripe(uuid, text, text, jsonb, integer, integer);
...
revoke all on function public.mark_order_paid_from_stripe(uuid, text, text, jsonb, integer, integer, text) from public, anon, authenticated;
grant execute on function public.mark_order_paid_from_stripe(uuid, text, text, jsonb, integer, integer, text) to service_role;
