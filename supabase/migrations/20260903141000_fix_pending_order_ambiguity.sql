-- PL/pgSQL treated product_id / size / quantity as columns in WHERE/INSERT.
-- Rename locals so create_pending_order can insert a test (or any) line.
...
begin
  ...
  loop
    ...
    if v_size not in ('XS', 'S', 'M', 'L', 'XL', 'XXL') then
