-- Numéros de commande humains (ex. DOYA-01001), uniques, pour SAV / mails / admin.
...
as $$
  select 'DOYA-' || lpad(nextval('public.order_number_seq')::text, 5, '0');
$$;

revoke all on function public.next_order_number() from public, anon, authenticated;
grant execute on function public.next_order_number() to service_role;

-- Backfill chronologique des commandes existantes.
with numbered as (
  select
    id,
    'DOYA-' || lpad((1000 + row_number() over (order by created_at asc, id asc))::text, 5, '0') as order_number
  from public.orders
...
-- Avancer la séquence au-delà du plus grand numéro déjà attribué.
select setval(
...
-- Recrée create_pending_order pour allouer + renvoyer orderNumber.
