-- Aligne le repo sur la fonction prod : taille U + auto-promos + shipping_quote + qty ≤ 6.
-- Remplace la régression introduite par 20260906192000_allow_unique_size_u.sql.
...
-- Aligne la contrainte order_items avec qty max 6 (front / edge / RPC).
alter table public.order_items drop constraint if exists order_items_quantity_check;
alter table public.order_items
  add constraint order_items_quantity_check check (quantity > 0 and quantity <= 6);
