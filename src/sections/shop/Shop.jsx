  ...
  return (
    <section id="shop" className="shop-section section-shell" aria-labelledby="shop-title">
      ...
      <div className="products">
        {products.map((product, index) => <Reveal as="article" className="product" key={product.id} delay={(index % 2) * 0.08}>
          <TransitionImage image={{ src: product[view], width: product.width, height: product.height }} className="product-image" alt={`${product.type} ${product.name}, ${product.color.toLowerCase()}, vue ${view === 'front' ? 'de face' : 'de dos'}.`} />
