...
test('JSON-LD MusicGroup / WebSite pointe vers le domaine', () => {
  ...
  assert.ok(types.includes('MusicAlbum'))
  assert.ok(types.includes('WebSite'))
  assert.ok(types.includes('WebPage'))
  const artist = graph['@graph'].find((node) => node['@type'] === 'MusicGroup')
  const album = graph['@graph'].find((node) => node['@type'] === 'MusicAlbum')
  assert.equal(artist.name, 'DOYA')
  assert.ok(Array.isArray(artist.sameAs) && artist.sameAs.length >= 5)
  assert.ok(Array.isArray(artist.member) && artist.member.length === 2)
  assert.equal(album.name, 'Luna Bohemia')
  assert.equal(album.numTracks, 12)
  assert.match(album.image, /luna-bohemia-cover\.jpg$/)
  assert.equal(album.track.length, 12)
})
